...
class Elements(object):
    ...
    def __init__(self, product_id = None, skin = None, *a, **k):
        ...
        self._skin = skin
        volume_encoder_touch_elements = [ self._make_note_button(13, index, u'Volume_Encoder_Touch_Element_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        tui_volume_sliders = [ SliderElement(MIDI_CC_TYPE, index + 1, 0, name=u'TUI_Volume_Slider_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        for slider in tui_volume_sliders:
            slider.set_feedback_delay(1)

        physical_volume_encoders = [ TouchEncoderElement(MIDI_CC_TYPE, 13, index, Live.MidiMap.MapMode.relative_smooth_two_compliment, name=u'Physical_Volume_Encoder_{}'.format(index), touch_element=volume_encoder_touch_elements[index]) for index in xrange(NUM_TRACK_CONTROLS) ]
        for encoder in physical_volume_encoders:
            encoder.set_feedback_delay(1)

        self.volume_sliders = ButtonMatrixElement(rows=[[ MultiElement(tui_volume_sliders[index], physical_volume_encoders[index]) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Volume_Controls')
        self.pan_sliders = ButtonMatrixElement(rows=[[ SliderElement(MIDI_CC_TYPE, index + 1, 1, name=u'TUI_Pan_Slider_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'TUI_Pan_Sliders')
        tui_device_controls = [ SliderElement(MIDI_CC_TYPE, 9, index, name=u'TUI_Device_Control_{}'.format(index)) for index in xrange(NUM_PARAM_CONTROLS) ]
        ...
        self.physical_device_control_touch_elements = ButtonMatrixElement(rows=[[ self._make_note_button(13, index + 8, u'Physical_Device_Control_Touch_Element_{}'.format(index)) for index in xrange(NUM_PARAM_CONTROLS) ]], name=u'Physical_Device_Control_Touch_Elements')
        physical_device_controls = [ EncoderElement(MIDI_CC_TYPE, 13, index + 8, Live.MidiMap.MapMode.relative_smooth_two_compliment, name=u'Physical_Device_Control_{}'.format(index)) for index in xrange(NUM_PARAM_CONTROLS) ]
        ...
        self.device_parameter_enable_controls = ButtonMatrixElement(rows=[[ self._make_note_button(9, index + 112, u'Device_Parameter_Enable_Control_{}'.format(index)) for index in xrange(NUM_PARAM_CONTROLS) ]], name=u'Device_Parameter_Enable_Controls')
        self.solo_buttons_raw = [ MultiElement(self._make_note_button(index + 1, 0, u'TUI_Solo_Button_{}'.format(index)), name=u'Solo_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        self.solo_buttons = ButtonMatrixElement(rows=[self.solo_buttons_raw], name=u'Solo_Buttons')
        self.mute_buttons_raw = [ MultiElement(self._make_note_button(index + 1, 1, u'TUI_Mute_Button_{}'.format(index)), name=u'Mute_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        self.mute_buttons = ButtonMatrixElement(rows=[self.mute_buttons_raw], name=u'Mute_Buttons')
        self.solo_mute_buttons_raw = [ MultiElement(self._make_note_button(index + 1, 2, u'TUI_Solo_Mute_Button_{}'.format(index)), name=u'Solo_Mute_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        self.solo_mute_buttons = ButtonMatrixElement(rows=[self.solo_mute_buttons_raw], name=u'Solo_Mute_Buttons')
        self.arm_buttons_raw = [ MultiElement(self._make_note_button(index + 1, 5, u'TUI_Arm_Button_{}'.format(index)), name=u'Arm_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        self.arm_buttons = ButtonMatrixElement(rows=[self.arm_buttons_raw], name=u'Arm_Buttons')
        self.clip_stop_buttons_raw = [ MultiElement(self._make_note_button(0, index + 16, u'Clip_Stop_TUI_Button_{}'.format(index)), name=u'Clip_Stop_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        ...
        self.send_encoders = ButtonMatrixElement(rows=[ [ EncoderElement(MIDI_CC_TYPE, row_index + 1, col_index + 3, Live.MidiMap.MapMode.absolute, name=u'TUI_Send_Encoder_{}_{}'.format(row_index, col_index)) for col_index in xrange(MAX_NUM_SENDS) ] for row_index in xrange(NUM_TRACK_CONTROLS) ], name=u'TUI_Send_Encoders')
        self.send_value_displays = ButtonMatrixElement(rows=[ [ self._create_text_display_element((1, row_index + col_index * NUM_TRACK_CONTROLS + 24), u'Send_Value_Display_{}_{}'.format(row_index, col_index)) for col_index in xrange(MAX_NUM_SENDS) ] for row_index in xrange(NUM_TRACK_CONTROLS) ], name=u'Send_Value_Displays')
        self.track_type_controls = ButtonMatrixElement(rows=[[ self._make_note_button(0, index + 8, u'Track_Type_Control_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Track_Type_Controls')
        ...
        self.meter_controls_left = ButtonMatrixElement(rows=[[ SliderElement(MIDI_CC_TYPE, index + 1, 124, send_should_depend_on_forwarding=False, name=u'Output_Meter_Display_Left_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Output_Meter_Displays_Left')
        self.meter_controls_right = ButtonMatrixElement(rows=[[ SliderElement(MIDI_CC_TYPE, index + 1, 125, send_should_depend_on_forwarding=False, name=u'Output_Meter_Display_Right_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Output_Meter_Displays_Right')
        self._tui_track_select_buttons_raw = [ self._make_note_button(0, index, u'TUI_Track_Select_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        self.track_name_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((0, index), u'Track_Name_Display_{}'.index) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Track_Name_Display')
        ...
        self.tui_track_color_controls = ButtonMatrixElement(rows=[[ ButtonElement(True, MIDI_CC_TYPE, 0, index + 16, skin=skin, name=u'TUI_Track_Color_Control_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'TUI_Track_Color_Controls')
        self.oled_display_style_controls_bank_1 = ButtonMatrixElement(rows=[[ self._make_note_button(13, index + 16, u'OLED_Display_Style_Control_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'OLED_Display_Style_Controls_Bank_1')
        self.oled_display_style_controls_bank_2 = ButtonMatrixElement(rows=[[ self._make_note_button(13, index + 24, u'OLED_Display_Style_Control_{}'.format(index + NUM_PARAM_CONTROLS)) for index in xrange(NUM_PARAM_CONTROLS) ]], name=u'OLED_Display_Style_Controls_Bank_2')
        self.track_name_or_volume_value_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((18, index + 16), u'Track_Name_Or_Volume_Value_Display_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Track_Name_Or_Volume_Value_Displays')
        self.device_parameter_name_or_value_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((18, index + 24), u'Device_Parameter_Name_Or_Value_Display_{}'.format(index)) for index in xrange(NUM_PARAM_CONTROLS) ]], name=u'Device_Parameter_Name_Or_Value_Displays')
        self.crossfade_assign_controls = ButtonMatrixElement(rows=[[ self._make_note_button(index + 1, 4, name=u'Crossfade_Assign_Controk_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Crossfade_Assign_Controls')
        self.crossfader = SliderElement(MIDI_CC_TYPE, 13, 16, name=u'Crossfader')
        self.physical_clip_launch_buttons_raw = [ [ self._make_note_button(12, col_index + self.physical_clip_launch_button_id_offset + row_index * NUM_TRACK_CONTROLS, u'Physical_Clip_Launch_Button_{}_{}'.format(col_index, row_index)) for col_index in xrange(NUM_TRACK_CONTROLS) ] for row_index in xrange(NUM_SCENE_CONTROLS) ]
        self.clip_launch_buttons_raw = [ [ MultiElement(self._make_note_button(0, col_index + 24 + row_index * NUM_TRACK_CONTROLS, u'TUI_Clip_Launch_Button_{}_{}'.format(col_index, row_index)), name=u'Clip_Launch_Button_{}_{}'.format(col_index, row_index)) for col_index in xrange(NUM_TRACK_CONTROLS) ] for row_index in xrange(NUM_SCENE_CONTROLS) ]
        self.clip_launch_buttons = ButtonMatrixElement(rows=self.clip_launch_buttons_raw, name=u'Clip_Launch_Buttons')
        self.scene_launch_buttons = ButtonMatrixElement(rows=[[ self._make_note_button(0, index + 88, name=u'TUI_Scene_Launch_Button_{}'.format(index)) for index in xrange(NUM_SCENE_CONTROLS) ]], name=u'TUI_Scene_Launch_Buttons')
        self.clip_name_displays = ButtonMatrixElement(rows=[ [ self._create_text_display_element((0, row_index + 16 + col_index * NUM_SCENE_CONTROLS), u'Clip_Name_Display_{}_{}'.format(col_index, row_index)) for col_index in xrange(NUM_TRACK_CONTROLS) ] for row_index in xrange(NUM_SCENE_CONTROLS) ])
        self.scene_name_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((0, index + 80), u'Scene_Name_Display_{}'.format(index)) for index in xrange(NUM_SCENE_CONTROLS) ]], name=u'Scene_Name_Displays')
        self.tui_scene_color_controls = ButtonMatrixElement(rows=[[ ButtonElement(True, MIDI_CC_TYPE, 0, index + 88, skin=skin, name=u'TUI_Scene_Color_Control_{}'.format(index)) for index in xrange(NUM_SCENE_CONTROLS) ]], name=u'TUI_Scene_Color_Controls')
        self.physical_clip_color_controls_raw = [ [ ButtonElement(True, MIDI_CC_TYPE, 12, row_index + self.pad_identifier_offset + col_index * NUM_SCENE_CONTROLS, skin=skin, name=u'Physical_Clip_Color_Control_{}_{}'.format(col_index, row_index)) for col_index in xrange(NUM_TRACK_CONTROLS) ] for row_index in xrange(NUM_SCENE_CONTROLS) ]
        self.clip_color_controls_raw = [ [ MultiElement(ButtonElement(True, MIDI_CC_TYPE, 0, row_index + 24 + col_index * NUM_SCENE_CONTROLS, skin=skin, name=u'TUI_Clip_Color_Control_{}_{}'.format(col_index, row_index)), name=u'Clip_Color_Control_{}_{}'.format(col_index, row_index)) for col_index in xrange(NUM_TRACK_CONTROLS) ] for row_index in xrange(NUM_SCENE_CONTROLS) ]
        self.clip_color_controls = ButtonMatrixElement(rows=self.clip_color_controls_raw, name=u'Clip_Color_Controls')
        self.playing_position_controls = ButtonMatrixElement(rows=[[ SliderElement(MIDI_CC_TYPE, 0, index, send_should_depend_on_forwarding=False, name=u'Playing_Position_Control_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Playing_Position_Controls')
        ...
        self.scene_selection_controls = ButtonMatrixElement(rows=[[ self._make_note_button(0, index + 96, u'Scene_Selection_Control_{}'.format(index)) for index in xrange(NUM_SCENE_CONTROLS) ]], name=u'Scene_Selection_Controls')
        ...
        self.tui_device_parameter_name_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((2, index + 16), u'TUI_Device_Parameter_Name_Display_{}'.format(index)) for index in xrange(NUM_PARAM_CONTROLS) ]], name=u'TUI_Device_Parameter_Name_Displays')
        self.tui_device_parameter_value_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((2, index + 32), u'TUI_Device_Parameter_Value_Display_{}'.format(index)) for index in xrange(NUM_PARAM_CONTROLS) ]], name=u'TUI_Device_Parameter_Value_Displays')
        ...
        self.volume_value_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((1, index), u'Volume_Value_Display_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Volume_Value_Displays')
        self.pan_value_displays = ButtonMatrixElement(rows=[[ self._create_text_display_element((1, 16 + index), u'Pan_Value_Display_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Pan_Value_Displays')
...
class ForceElements(Elements):
    ...
    def __init__(self, *a, **k):
        ...
        self.track_assign_buttons_raw = [ self._make_note_button(12, index + 8, u'Track_Assign_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        self.track_assign_buttons = ButtonMatrixElement(rows=[self.track_assign_buttons_raw], name=u'Track_Assign_Buttons')
        self.track_assign_color_controls_raw = [ ButtonElement(True, MIDI_CC_TYPE, 12, index + 32, skin=self._skin, name=u'Track_Assign_Color_Control_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        ...
        physical_track_select_buttons_raw = [ self._make_note_button(12, index, u'Physical_Track_Select_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]
        self.track_select_buttons = ButtonMatrixElement(rows=[[ MultiElement(self._tui_track_select_buttons_raw[index], physical_track_select_buttons_raw[index], name=u'Track_Select_Button_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Track_Select_Buttons')
        self.physical_track_select_buttons_with_shift = ButtonMatrixElement(rows=[[ ComboElement(physical_track_select_buttons_raw[index], modifier=[self.shift_button], name=u'Physical_Track_Select_Button_With_Shift') for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Physical_Track_Select_Buttons_With_Shift')
        self.physical_track_color_controls = ButtonMatrixElement(rows=[[ ButtonElement(True, MIDI_CC_TYPE, 12, index + 24, skin=self._skin, name=u'Physical_Track_Color_Control_{}'.format(index)) for index in xrange(NUM_TRACK_CONTROLS) ]], name=u'Physical_Track_Color_Controls')
        self.force_physical_scene_launch_buttons = ButtonMatrixElement(rows=[[ self._make_note_button(12, index + 80, name=u'Force_Physical_Scene_Launch_Button_{}'.format(index)) for index in xrange(NUM_SCENE_CONTROLS) ]], name=u'Force_Physical_Scene_Launch_Buttons')
...
class MPCElementsBase(Elements):
    ...
    def __init__(self, *a, **k):
        ...
        self.mpc_scene_color_controls = ButtonMatrixElement(rows=[ [ col[row_index] for col in self.physical_clip_color_controls_raw[:4] ] for row_index in xrange(4) ], name=u'MPC_Scene_Color_Controls')
