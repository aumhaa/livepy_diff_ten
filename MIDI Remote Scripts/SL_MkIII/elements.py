...
def create_display_color_element_line(v_position, **k):
    raw_line = [ create_display_color_element(index, v_position, u'Color_Field_{}_{}'.format(index, v_position), **k) for index in xrange(SESSION_WIDTH) ]
...
class Elements(object):
    ...
    def __init__(self, *a, **k):
        ...
        pads_raw = [ [ create_button(offset + col_index, u'Pad_{}_{}'.format(col_index, row_index), msg_type=MIDI_NOTE_TYPE) for col_index in xrange(SESSION_WIDTH) ] for row_index, offset in enumerate((96, 112)) ]
        ...
        scene_launch_buttons_raw = [ create_button(83 + row_index, name=u'Scene_Launch_Button_{}'.format(row_index)) for row_index in xrange(SESSION_HEIGHT) ]
        ...
        sliders = [ SpecialFeedbackChannelSliderElement(MIDI_CC_TYPE, DEFAULT_CHANNEL, 41 + index, name=u'Slider_{}'.format(index)) for index in xrange(SESSION_WIDTH) ]
        for slider in sliders:
            slider.set_feedback_delay(1)

        self.sliders = ButtonMatrixElement(rows=[sliders], name=u'Sliders')
        self.slider_leds = ButtonMatrixElement(rows=[[ create_rgb_led(index + 54, u'Slider_LED_{}'.format(index)) for index in xrange(8) ]], name=u'Slider_LEDs')
        self.mixer_soft_button_row_0 = ButtonMatrixElement(rows=[[ create_button(identifier, u'Mixer_Soft_Button_{}_0'.format(col_index)) for col_index, identifier in enumerate(xrange(59, 59 + SESSION_WIDTH)) ]], name=u'Mixer_Soft_Button_Row_0')
        self.mixer_soft_button_row_1 = ButtonMatrixElement(rows=[[ create_button(identifier, u'Mixer_Soft_Button_{}_1'.format(col_index)) for col_index, identifier in enumerate(xrange(59 + SESSION_WIDTH, 59 + SESSION_WIDTH * 2)) ]], name=u'Mixer_Soft_Button_Row_1')
        ...
        self.select_buttons_raw = [ create_button(51 + index, u'Select_Button_{}'.format(index)) for index in xrange(8) ]
        ...
        encoders = [ EncoderElement(MIDI_CC_TYPE, DEFAULT_CHANNEL, 21 + index, map_mode=Live.MidiMap.MapMode.relative_smooth_two_compliment, name=u'Encoder_{}'.format(index)) for index in xrange(8) ]
        for encoder in encoders:
            encoder.set_feedback_delay(1)

        self.encoders = ButtonMatrixElement(rows=[encoders], name=u'Encoders')
        self.encoder_color_fields = ButtonMatrixElement(rows=[[ create_display_color_element(index, 1, u'Encoder_Color_Field_{}'.format(index)) for index in xrange(8) ]], name=u'Encoder_Color_Fields')
